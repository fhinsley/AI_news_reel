#!/usr/bin/env python3
"""Generate an SRT caption file from the debate clip timestamp data.

Structural mirror of scripts/generate_srt.py.

Reads each clip's timestamp JSON and DEBATE_CLIP_MANIFEST to reconstruct
the full debate timeline — including the silent framing card gap — then
groups characters into caption chunks and writes a standard SRT file.

Pipeline position: run after build_video.py.
Output: <WEEK_FOLDER>/DebateCaptions.srt
"""
...
# ---------------------------------------------------------------------------
# Output path — parallel to config.SRT_OUTPUT_FILE
# ---------------------------------------------------------------------------

SRT_OUTPUT_FILE = Path(config.project_path(config.WEEK_FOLDER_NAME, "DebateCaptions.srt"))

# ---------------------------------------------------------------------------
# Helpers — identical to generate_srt.py
# ---------------------------------------------------------------------------
...
def chunk_words(words, target=config.SRT_TARGET_WORDS, max_duration=config.SRT_MAX_DURATION):
    """Group words into caption-sized chunks. Identical to generate_srt.py."""
    chunks  = []
    current = []
...
def build_srt_entries(chunks, time_offset: float):
    """Convert word chunks to (start, end, text) triples. Identical to generate_srt.py."""
    entries = []
    for chunk in chunks:
        if not chunk:
            continue
...
def main() -> int:
    ...
    for stem, label in config.DEBATE_CLIP_MANIFEST:
        ...
        # Insert framing card silence before the responder — mirrors stitch_audio()
        # in both tts.py and build_video.py
    ...
    SRT_OUTPUT_FILE.write_text("\n".join(lines), encoding="utf-8")
    print(f"\n{len(all_entries)} captions written to: {SRT_OUTPUT_FILE}")
    return 0
